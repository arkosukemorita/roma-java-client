 ...
 * 
 */
public class New_StringWrapper {

    protected New_RomaClient client;

    public New_StringWrapper(New_RomaClient client) {
	if (client == null) {
    ...
    public New_RomaClient getRomaClient() {
	return this.client;
    }
	...
	try {
	    return client.append(key, value.getBytes(StringUtil.ENCODING),
		    expiry);
    ...
    public boolean append(String key, String value, long expiry)
	    throws ClientException {
	try {
	    return client.append(key, value.getBytes(StringUtil.ENCODING),
		    expiry);
    ...
    public boolean prepend(String key, String value, Date expiry)
	    throws ClientException {
	try {
	    return client.prepend(key, value.getBytes(StringUtil.ENCODING),
		    expiry);
	} catch (UnsupportedEncodingException e) {
	    throw new ClientException(e);
	}
    ...
    public boolean prepend(String key, String value, long expiry)
	    throws ClientException {
	try {
	    return client.prepend(key, value.getBytes(StringUtil.ENCODING),
		    expiry);
    ...
    public boolean put(String key, String value, long expiry)
	    throws ClientException {
	try {
	    return client.put(key, value.getBytes(StringUtil.ENCODING), expiry);
	} catch (UnsupportedEncodingException e) {
