...
public class RomaClientImplTest_longExpire extends TestCase {

    private static String NODE_ID = AllTests.NODE_ID;

    private static String KEY_PREFIX = RomaClientImplTest_longExpire.class
	    .getName();
    ...
    public RomaClientImplTest_longExpire() {
	super();
    }
	...
	try {
	    ...
	    // assertTrue(CLIENT.put(KEY + "01", "01".getBytes(), new
	    // Date(2000)));
	    assertTrue(CLIENT.put(KEY + "01", "01".getBytes(), 2));
	    assertTrue(CLIENT.put(KEY + "02", "02".getBytes()));
	    assertTrue(CLIENT.put(KEY + "03", "03".getBytes()));
	    List<String> keys = new ArrayList<String>();
	    keys.add(KEY + "01");
	...
	assertTrue(CLIENT.append(KEY, "03".getBytes(), 2));
	byte[] ret = CLIENT.get(KEY);
	assertEquals("010203", new String(ret));
	Thread.sleep(3000);
	ret = CLIENT.get(KEY);
	...
	assertTrue(CLIENT.prepend(KEY, "03".getBytes(), 2));
	byte[] ret = CLIENT.get(KEY);
	assertEquals("030201", new String(ret));
	Thread.sleep(3000);
	ret = CLIENT.get(KEY);
	...
	try {
	    ...
	    assertEquals(CasResponse.OK, CLIENT.cas(KEY + "01", values.get(
		    KEY + "01").getCas(), "001".getBytes()));
	    assertEquals(CasResponse.NOT_FOUND, CLIENT.cas(KEY + "04", values
